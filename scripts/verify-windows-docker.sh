...
echo "== 3. CROSS-COMPILE concierge-cli.exe (Windows PE) =="
...
EXE=/work/target/x86_64-pc-windows-gnu/debug/concierge-cli.exe
file "$EXE" | sed 's/^/  /'
echo "== 4. RUN THE WINDOWS BINARY UNDER WINE =="
export WINEDEBUG=-all WINEPREFIX=/tmp/wp; wineboot -i >/dev/null 2>&1 || true
echo "  concierge-cli.exe --help:"; wine "$EXE" --help 2>/dev/null | head -3 | sed 's/^/    /'
